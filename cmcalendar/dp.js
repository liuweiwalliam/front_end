/**
 * Created by walliam on 2017/1/13.
 */
...
Date.fullYearStart = "20"; (function() {
        ...
        renderCalendar: function(t) {
            ...
            while (n++<q) {
                ...
                for (var x = 0; x < 7; x++) {
                    ...
                    //-----------
                    // 获取农历 2014.03.27
                    var nonglDate = new ChineseCalendar(m).solarDay2(); //获取农历日期
                    var nl1=nonglDate.charAt(nonglDate.length - 1);
                    var nl2=nonglDate.charAt(nonglDate.length - 2);
                    var nl3=nonglDate.charAt(2);
                    var nnl=nl2+nl1;
                    var yue=parseInt(nonglDate.substr(0,2));

                    var nongli = new ChineseCalendar(m).cDay(yue,nnl); //获取农历日期
                    var nongli2 = new ChineseCalendar(m).cDay2(nnl);
                    var nonglstr="";
                    if (t.isnongLi) {
                        nonglstr="<em class='s_yinl'>"+nongli2+ "</em>";
                    }
                    //-----------

                    var y = d(C("td")).html("<div class=\"td_td \" da=\""+m+"\" da-nl=\""+nongli+"\" da-wk=\""+(new ChineseCalendar(m)).weekday()+"\">"+nonglstr+"<span class='s_day'>"+m.getDate() + "</span></div>").addClass((j ? "current-month ": "other-month ") + (m.isWeekend() ? "weekend ": "weekday ") + (j && m.getTime() == z.getTime() ? "today ": "")).data("datePickerDate", m.asString()).hover(A(k), g);
    ...
    d.extend(a.prototype, {
        init: function(e) {
            ...
            this.showMonNavigation = e.showMonNavigation;
            this.isnongLi = e.isnongLi;
        ...
        display: function(e) {
            ...
            if (this.showMonNavigation == false) { //xin
                d("#dp-nav-prev-month, #dp-nav-next-month", m.context).css("display", "none")
            }
        ...
        _displayNewMonth: function(f, e, g) {
            if (!d(f).is(".disabled")) {
                ...
                if( typeof getM === 'function' ){
                    getM(this.displayedMonth,this.displayedMonth+1);  // add my
                }
...
/*
 2017.1.9 新增 显示下一月上一月控制参数 showMonNavigation
 2017.1.9 新增 是否显示农历参数 isnongLi
 */